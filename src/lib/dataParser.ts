...
export function parseUploadedData(rows: ParsedRow[]): Partial<CompanyData> {
    ...
    rows.forEach((row) => {
        let questionId =
            row.questionId || row['Question ID'] || row['QuestionID'];
...
function getExampleAnswer(mapping: any): string {
    switch (mapping.questionId) {
        ...
        case 'Q003':
            return 'Leading European food manufacturer specializing in ambient grocery products';
        case 'Q013':
            return 'John Smith|CEO|Former P&G executive with 15 years experience|Jane Doe|CFO|Ex-Unilever finance director';
        case 'Q014':
            return 'Ambient Foods|Biscuits, crackers, snacks|Dairy Products|Milk, cheese, yogurt';
        case 'Q016':
            return '2023|€2.1B|€180M|€120M|8.6%|€95M|€450M|2022|€1.9B|€160M|€100M|8.4%|€80M|€500M';
        default:
            return 'Example answer for ' + mapping.question;
    }
}

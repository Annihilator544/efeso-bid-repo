...
export function SlideLayout({ 
  ...
  return (
    <div className={`relative w-full h-fit min-h-[600px] bg-white border border-gray-200 shadow-lg ${className}`}>
      ...
      {/* Main content area */}
      <div className={`${title && !isTitle ? "px-8 pb-8" : "p-8"} ${isTitle ? "flex flex-col items-center justify-center h-full" : ""}`}>
        {children}
      </div>
      ...
      {!isTitle && (
        <div className="absolute bottom-0 left-0 right-0 flex justify-between items-center p-6 text-xs text-gray-500">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-gray-600">EFESO UK BID</span>
          </div>
